...
export function useFontWasm() {
  ...
  async function spliceFontsAtCuts(
    secondFontData: Uint8Array,
    spliceResult: SpliceLLMResult,
  ): Promise<Uint8Array | null> {
    if (!fontData.value) return null
    const wasm = await loadWasm()
    const { default: def, perChar } = spliceResult
    const request = JSON.stringify({
      cut1: def.cut1,
      cut2: def.cut2,
      zones: def.zones,
    ...
    const bytes: Uint8Array = wasm.splice_fonts_at_cuts(fontData.value, secondFontData, request)
    return bytes
  }
  ...
  return { fontData, fontInfo, isLoading, error, loadFont, styleFont, blendFontsSdfCanvas, spliceFontsAtCuts, resetFont }
}
